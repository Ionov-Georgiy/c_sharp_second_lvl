...
namespace AsteroidGame
{
    class Star : ObjectInSpace
    {
        ...
        public Star(Point Position, Point Direction, int Size, StarType StarType, int StarBrightness)
        ...
        public override void Move(Size SpaceSize)
        {
            Random rand = new Random();
            Position.X += Direction.X;
            if (Position.X < 0)
            {
                Position.X = SpaceSize.Width + ObjectSize.Width;
                Position.Y = rand.Next(ObjectSize.Height/2, SpaceSize.Height - ObjectSize.Height/2);
            }
        }
    }
}
